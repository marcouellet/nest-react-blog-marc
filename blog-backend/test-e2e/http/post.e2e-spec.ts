...
describe('PostController (e2e)', () => {
  ...
  it('POST(11): (POST) /category/create - Submit a new category (dummy logged in)', () => {
  ...
  it('POST(12b): (GET) /post/count/user/:userId - Get number of posts owned by user with dummy userId (logged not required)', () => {
    ...
    return request(app.getHttpServer())
      .get(`/post/count/user/${dummyUserDtoWithTokens.id}`)
      .expect(StatusCodes.OK)
      .expect(response => response && JSON.stringify(response.body) === JSON.stringify(testPostCount)) // Should be 1
      .catch(error => {
